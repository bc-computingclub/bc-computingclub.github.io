...
const b_newFolder = document.querySelector(".b-new-folder");
const b_newFile = document.querySelector(".b-new-file");
b_newFolder.addEventListener("click", e => {
    ...
    new InputMenu("New Folder", "Enter folder name", (v) => {
        if (!v)
            return;
...
b_newFile.addEventListener("click", e => {
    ...
    new InputMenu("New File", "Enter file name", (inputname) => {
        if (!inputname)
            return;
...
async function loadProject(uid, pid) {
    project = null;
    ...
    if (project.curFile)
        project.curFile.open();
    // load index.html by default
    let index = project.items.find(v => v.name == "index.html");
    if (index)
        index.open();
    if (!project.canEdit)
...
class ProjectDashboard extends Menu {
    ...
    load(priority) {
        ...
        b_newProject.addEventListener("click", e => {
            ...
            new InputMenu("New Project", "Enter project name", (projname) => {
                name = projname;
                if (!name)
                    return;
    ...
    loadItem(meta) {
        ...
        b_ops.addEventListener("click", e => {
            ...
            if (cur)
                div.insertAdjacentElement("beforebegin", cur);
        });
...
async function createNewProject(name, desc = "A project for experiments.") {
...
async function openProjectSuper(pid) {
    if (!project)
        await loadProject(g_user.uid, pid);
    else
        goToProject(pid);
    closeAllMenus();
}
...
function openCurProjSettings() {
    ...
    div.onclose = function () {
        ...
        if (oldName == newName)
            return;
        socket.emit("renameProject", project.pid, newName, (res) => {
...
async function deleteProject(meta, div) {
    return new Promise(resolve => {
        new ConfirmMenu("Delete Project", "Are you sure you want to delete project:<br>\"" + meta.name + "\" ?<br><br>There is no reversing this option.", () => {
            socket.emit("deleteProject", meta.pid, (res) => {
                if (res != 0) {
                    resolve(false);
                ...
                if (div)
                    div.remove();
                resolve(true);
                if (project?.pid == meta.pid)
                    location.reload();
            });
        }, () => { return; }).load();
    });
}
async function unlinkProject(meta) {
    return new Promise(resolve => {
        new ConfirmMenu("Unlink Project", "Are you sure you want to unlink the challenge with this project:<br>\"" + meta.name + "\" ?<br><br>There is no reversing this option.", () => {
            socket.emit("unlinkProject", meta.pid, (res) => {
                if (res != 0) {
                    resolve(false);
                ...
                resolve(true);
                if (project?.pid == meta.pid)
                    location.reload(); // caleb: I put this one back since you don't want to reload when unlinking a project that isn't open right now
                // location.reload();
            });
        }, () => { return; }).load();
    });
}
let curProjectSettingsMetaPID;
let curProjectSettingsMeta;
function openCurProjSettingsMeta(meta, ops) {
    if (!meta)
        return;
    ...
    whenEnter(i_rename, v => {
        ...
        if (oldName == newName)
            return;
        socket.emit("renameProject", meta.pid, newName, (res) => {
            console.log("rename project res:", res);
            let elm = document.querySelector(".d-current-project");
            if (elm)
                elm.textContent = newName;
    ...
    let b_deleteProject = div.querySelector(".b-delete-project");
    ...
    let b_unlinkProject = div.querySelector(".b-unlink-project");
    if (!meta.cid)
        b_unlinkProject.style.display = "none";
...
class ProjSettingsMenu extends Menu {
    ...
    load(priority) {
        ...
        if (this.meta.cid == null)
            this.menu.classList.add("experiment-menu");
        else
            this.menu.classList.add("practice-menu");
        this.body.innerHTML = `
        ...
        `;
        let i_name = this.body.querySelector(".i-name");
        let i_desc = this.body.querySelector(".i-desc");
        let b_save = this.body.querySelector(".b-save-changes");
        let b_privacy = this.body.querySelector(".b-toggle-privacy");
        let b_deleteProject = this.body.querySelector(".b-delete-project");
        let b_unlinkProject = this.body.querySelector(".b-unlink-project");
        i_name.value = this.meta.name;
        i_desc.value = this.meta.desc;
        i_name.addEventListener("input", e => {
            if (i_name.value == "" || i_name.value == this.meta.name)
                b_save.disabled = true;
            else
                b_save.disabled = false;
        });
        i_desc.addEventListener("input", e => {
            if (i_desc.value == this.meta.desc)
                b_save.disabled = true;
            else
                b_save.disabled = false;
        });
        ...
        b_privacy.addEventListener("click", e => {
            ...
            let _starTime = performance.now();
            socket.emit("setProjVisibility", this.meta.pid, !this.meta.isPublic, async (res, v) => {
                let time = performance.now() - _starTime;
                let timeLimit = 300;
                if (time < timeLimit)
                    await wait(timeLimit - time);
        ...
        b_deleteProject.addEventListener("click", async (e) => {
            await deleteProject(this.meta, this.divItem);
            if (this.dash)
                this.dash.reloadSection();
            this.close();
        });
        if (b_unlinkProject)
            b_unlinkProject.addEventListener("click", async (e) => {
