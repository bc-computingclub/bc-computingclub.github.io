...
b_publish.addEventListener("click", e => {
    ...
    if (!project.meta.submitted) {
        new ConfirmMenu("Submit Challenge", "Are you sure you want to submit your code?<br><br> You will have to un-submit it to make any changes in the future.<br><br><span class='note'>Submitted projects are publically viewable by anyone.</span>", () => {
        ...
        new ConfirmMenu("Remove submission", "Are you sure you'd like to un-submit your challenge?<br><br> You'll have to re-submit in order for it to appear in the submissions page.", () => {
...
let b_openInNew = document.querySelector(".b-open-in-new");
let b_invertColors = document.querySelector(".b-invert-colors");
b_openInNew.addEventListener("click", e => {
    if (PAGE_ID == PAGEID.editor)
        open(project.getExternalURL(), "_blank");
    else if (PAGE_ID == PAGEID.lesson)
        open(lesson.p.getExternalURL(), "_blank");
});
...
if (settings.invertContrast.get())
    updateIFrameContrast();
setTimeout(() => {
    initCallouts();
}, 500);
//# sourceMappingURL=post_editor.js.map