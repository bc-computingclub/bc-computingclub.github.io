...
class Section{
    ...
    buildHome(){
        function add(s:Section,path:Section[]){
            s.path.push(...path,s);
...
let lCrumbs = document.querySelector(".lesson-crumbs") as HTMLElement;
...
function createProgSection(s:Section){
...
function createProgLesson(l:Lesson){
    let div = document.createElement("div");
...
function createProgDivFinalize(div:HTMLElement){
...
function loadProgList(){
    ...
    if(curSection) for(let i = 0; i < curSection.list.length; i++){
        let a = curSection.list[i];
        if(a instanceof Lesson) createProgLesson(a);
        else if(a instanceof Section) createProgSection(a);
    }
}
