...
function logUserIn(data?: CredentialResData, token?: string) {
  if (!data)
    data = JSON.parse(localStorage.getItem("logData")) as CredentialResData;
...
function genHeader(i: number, isCompact = true, id: string) {
  ...
  document.body.classList.add(
    ["learn-page", "practice-page", "experiment-page"][i],
  );
...
class Project {
  constructor(
    title: string,
    parent: HTMLElement,
    settings: { readonly?: boolean; disableCopy?: boolean } = {},
  ) {
    this.title = title;
    this.items = [];
    this.files = [];
  ...
  async deleteFItem(f: FItem) {
    ...
    if (
      !confirm(
        ...
        "] ?\n\nThere is no reversing this option!",
      )
    )
      return;
  ...
  copyFItems(f: FItem) {
    ...
    f.p.deselectHLItems();
    console.log(
      "Copied " + fclipboard.files.length + " items into the clipboard",
    );
  }
  ...
  createFile(
    ...
    if (same) {
      if (same instanceof FFile) {
        if (
          confirm(
            "There is already a file in this folder with the name: " +
            name +
        ...
        alert(
          "There is already a folder in this location with the name: " +
          name +
          ", skipping...",
        );
        return;
...
function sortFiles(l: any[]) {
  l.sort((a, b) => {
    let dif =
      (a.val != null || a.curI != null) && b.val == null && b.curI == null
        ? 1
        : a.val == null && a.curI == null && (b.val != null || b.curI != null)
          ? -1
          : 0;
...
function setupFItemDropdown(f: FItem, div: HTMLElement) {
  setupDropdown(
    ...
    () => {
      ...
      return ["Rename", "Delete", cut, cpy, paste];
    },
    async (i) => {
      function close() {
        closeAllSubMenus();
...
async function moveFile(f: FItem, toFolder: FFolder, noSave = false) {
  ...
  let res = await new Promise<number>((resolve) => {
    socket.emit(
      ...
      f.p.pid,
      [f.name],
      calcFolderPath(fromFolder),
      calcFolderPath(toFolder),
      (res: any) => {
...
class FFile extends FItem {
  ...
  setSaved(v: boolean, noChange = false) {
    ...
    if (!noChange)
      if (this.editor) this._lastSavedText = this.editor.getValue();
  }
  ...
  close() {
    if (this.p.openFiles.includes(this)) {
      if (!this._saved) {
        if (
          !confirm("This file is unsaved, are you sure you want to close it?")
        )
          return;
      }
  ...
  open(isTemp?: boolean) {
    ...
    if (!this.p.openFiles.includes(this)) {
      ...
      this.p.d_files.insertBefore(
        link,
        this.p.d_files.children[this.p.d_files.children.length - 2],
      );
        ...
        dragItem = new DragData<FFile>(
          ...
          (last) => {
            ...
            if (i1 >= i2)
              ...
              project.d_files.insertBefore(
                last.link,
                hoverOpenFile.link.nextElementSibling,
              );
          },
        );
        dragItem.down = this;
...
function setupEditor(parent: HTMLElement, type: EditorType) {
  ...
  if (type != EditorType.none) {
    ...
    label.className =
      type == EditorType.tutor ? "editor-type-tutor" : "editor-type-self";
...
function postSetupEditor(project: Project) {
  ...
  add_file.addEventListener("click", (e) => {
    ...
    project.createFile(
      ...
      "",
      null,
      project.lastFolder ?? project.curFile?.folder,
      true,
    );
  });
  ...
  if (ops_rename) {
    ...
    ops_cut.addEventListener("click", (e) => {
      if (!project) return;
      let items = project.hlItems.length
        ? project.hlItems
        : [project.lastFolder ?? project.curFile];
...
async function refreshProject() {
  ...
  if (
    project.needsSave ||
    project.files.some((v) => !v._saved) ||
    !project.hasSavedOnce
  ) {
  ...
  if (!file1 || !project.hasSavedOnce) {
    alert(
      "No index.html file found! Please create a new file called index.html in the outer most folder of your project, this file will be used in the preview.",
    );
    return;
  }
  iframe.src =
...
async function refreshLesson() {
  ...
  if (!file1 || !project.hasSavedOnce) {
    alert(
      "No index.html file found! Please create a new file called index.html, this file will be used in the preview.",
    );
    return;
  }
  iframe.src =
...
function setupCallout(ele: Element, label?: string) {
  ...
  ele.addEventListener("mouseenter", async (e: MouseEvent) => {
    ...
    if (rect.x > innerWidth / 2) {
      ...
      d.style.setProperty(
        "--left",
        rect.width / 2 - 7 - rect.width + d.offsetWidth + "px",
      );
