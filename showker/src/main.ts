let getStartedButton = document.querySelector(".co-button") as HTMLButtonElement;
let imageDiv = document.querySelector(".showcase-cont") as HTMLElement;
let images = document.querySelectorAll(".showcase-img") as NodeListOf<HTMLImageElement>;
let codeOtterTitle = document.querySelector(".co-top") as HTMLElement;
let i:number = 0;
...
function updateFeatures() {
    if(!codeOtterTitle.classList.contains("altpos")) {codeOtterTitle.classList.add("altpos")};
...
function goNext() {
    ...
    } else alert("You have reached the end of the presentation.");
}
...
function goPrev() {
    if(i > 1) {
        i -= 2;
        imageDiv.classList.add("removeimgdiv");
    } else alert("You are at the beginning of the presentation.");
}
